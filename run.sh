...
OUTPUT_FILE="/home/peterkad/pkadmaster/indel_scanner/results/test/ph/15-12-2025-ph_indels"
...
python -m cProfile -o profile_output.prof -m indel_scanner.main scan\
    --bam "$BAM_FILE" \
    --fasta "$FASTA_FILE" \
    --output "$OUTPUT_FILE" \
    --config "$CONFIG_FILE" \
