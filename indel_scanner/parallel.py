...
def parallel_pipeline(scannerconfig: PipelineConfig) -> tuple[Path, int, dict]:
    ...
    pre_callable_lengths = list(range(1, 11))
    scannerconfig.callable_lengths = pre_callable_lengths
    logger.info(
        "Starting callable-bases pass for 1-10bp in parallel with scan using %s workers...",
        callable_workers,
    )
    callable_pool = Pool(
    ...
    for result in pre_results:
        if result:
            _, stats = result
            pre_total_aligned += stats["total_aligned_bases"]
    ...
    total_stats["total_aligned_bases"] += pre_total_aligned
    total_stats["sampling_bases_total"] += pre_sampling_total
    for k, v in pre_passable_by_type.items():
    ...
    post_callable_lengths = [length for length in observed_lengths if length > 10]

    if post_callable_lengths:
        scannerconfig.callable_lengths = post_callable_lengths
        logger.info(
            "Starting callable-bases pass for >10bp with %s workers...",
            callable_workers,
        )
        status_queue = Queue()
        ui_queue = Queue()
                ...
                with Pool(
                    ...
                    for result in pool.imap_unordered(_process_contig_callable, contigs):
                        if result:
                            _, stats = result
                            if stats.get("sampled_contig"):
                                post_sampling_total += stats["sampling_bases_total"]
                                for k, v in stats["sampling_passable_by_type"].items():
        ...
        total_stats["sampling_bases_total"] += post_sampling_total
        for k, v in post_passable_by_type.items():
            total_stats["sampling_passable_by_type"][k] = (
                total_stats["sampling_passable_by_type"].get(k, 0) + v
            )
