...
@pytest.mark.parametrize("seq, interval, min_len, expected", [
    ...
    ("AAACCCGGG", (0, 1), 3, True), # indel starts spanning homopolymer into suffix: [AA]ACCCGGG
    ...
    ("AAACCCGGG", (0, 2), 4, False), # Run exists, but is shorter than min_len=4: [AAA]CCCGGG
])
...
@pytest.mark.parametrize("prefix, indel, suffix, expected_filter", [
    # Should be filtered
    ...
    ("GATT", "AC", "CCT", True),  # Indel starts a spanning homopolymer into the suffix
    ("GAA", "AT", "TACC", True), #Indel ends a spanning homopolymer

    # Should NOT be filtered
    ("GATT", "AC", "TACC", False),  # No homopolymers, TODO: filter more stricly when indel is in homopolymer
